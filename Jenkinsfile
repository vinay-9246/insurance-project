node{
    ...
    stage('git code checkout'){
        try{
            ...
            ${BUILD_URL}''', subject: 'Job ${JOB_NAME} ${BUILD_NUMBER} is failed', to: 'vishwakarmasadhana1996@gmail.com'
        }
    }
    ...
    stage('Containerize the application'){
        ...
        sh "${dockerCMD} build -t sadhana1996/insure-me:${tagName} ."
    }
    
    stage('Pushing it ot the DockerHub'){
        echo 'Pushing the docker image to DockerHub'
        withCredentials([string(credentialsId: 'dock-password', variable: 'dockerHubPassword')]) {
        sh "${dockerCMD} login -u sadhana1996 -p ${dockerHubPassword}"
        sh "${dockerCMD} push sadhana1996/insure-me:${tagName}"
